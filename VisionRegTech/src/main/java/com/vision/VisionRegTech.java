...
public class VisionRegTech {
	...
	public CommandLineRunner commandLineRunner(ApplicationContext appContext) {
		...
		/*
		 * long ut1 = Instant.now().getEpochSecond(); Date dNow = new Date(ut1);
		 * Calendar cal = Calendar.getInstance(); cal.setTime(dNow);
		 * cal.add(Calendar.MINUTE, 5); dNow = cal.getTime();
		 */
...
//			String[] arr = Arrays.stream(allowedOrigins.split(",")).map(String::trim).filter(s -> !s.isEmpty())
//					.toArray(String[]::new);
//			for (String o : arr) {
