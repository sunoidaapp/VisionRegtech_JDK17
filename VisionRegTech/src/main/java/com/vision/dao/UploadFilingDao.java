...
public class UploadFilingDao extends AbstractDao<UploadFilingVb> {
	...
	public List<UploadFilingVb> getQueryResults(UploadFilingVb dObj) {
		...
		if ("MSSQL".equalsIgnoreCase(databaseType)) {
			strBufApprove = new StringBuffer(
					"Select TAppr.COUNTRY, TAppr.LE_BOOK, TAppr.FILE_NAME, TAppr.TEMPLATE_NAME, "
		...
		try {
			 ...
			 * CommonUtils.addToQuery("TAppr.UPLOAD_DATE = CONVERT(datetime, ?, 103) ",
			 * strBufApprove);
			 * 
			 * }
			 */
			...
			if (ValidationUtil.isValid(SessionContextHolder.getContext().getUpdateRestrictionLeBook())) {
				if ("MSSQL".equalsIgnoreCase(databaseType))
					CommonUtils.addToQuery("TAppr.COUNTRY+ '-' +TAppr.LE_BOOK IN("
	...
	public List<UploadFilingVb> getUploadFillingQueryResults(UploadFilingVb dObj) {
		...
		if ("MSSQL".equalsIgnoreCase(databaseType)) {
			strQueryAppr = new StringBuffer(
					" SELECT T1.COUNTRY, T1.LE_BOOK, T1.TEMPLATE_NAME, (T1.TEMPLATE_NAME + '-' + T2.GENERAL_DESCRIPTION) TEMPLATE_NAME_DESC, "
							...
							+ " WHERE AST.ALPHA_TAB = T3.ADF_SCHEDULE_STATUS_AT  " + " AND AST.ALPHA_SUB_TAB =  "
		...
		if (ValidationUtil.isValid(SessionContextHolder.getContext().getUpdateRestrictionLeBook())) {
			if ("MSSQL".equalsIgnoreCase(databaseType))
				strQueryAppr.append("AND T1.COUNTRY+ '-' +T1.LE_BOOK IN("
						+ SessionContextHolder.getContext().getUpdateRestrictionLeBook().toUpperCase() + ")");
			else
				strQueryAppr.append("AND T1.COUNTRY|| '-' ||T1.LE_BOOK IN("
						+ SessionContextHolder.getContext().getUpdateRestrictionLeBook().toUpperCase() + ")");
		...
		try {
			return getQueryPopupResults(dObj, new StringBuffer(), strQueryAppr, new String(), orderBy, params,
					getUploadFillingMapper());
		} catch (Exception ex) {
			ex.printStackTrace();
			logger.error("Error: getQueryResults Exception :   ");
			logger.error(((strQueryAppr == null) ? "strQueryAppr is Null" : strQueryAppr.toString()));
	...
	public List<UploadFilingVb> getQueryResults(UploadFilingVb dObj, int intStatus) {
		...
		if ("MSSQL".equalsIgnoreCase(databaseType)) {
			strQueryAppr = new StringBuffer(" Select T1.COUNTRY, T1.LE_BOOK, "
					...
					+ " CASE WHEN (SELECT COUNT (1) FROM ADF_PROCESS_CONTROL APC WHERE APC.ADF_NUMBER = T1.ADF_NUMBER AND (APC.EOD_INITIATED_FLAG = 'Y' OR APC.AUTH_STATUS in (0,4) )) > 0 THEN 'Y' ELSE 'N' END AS EOD_INITIATED_FLAG, "
					+ " ISNULL(T3.NODE_OVERRIDE,T3.NODE_REQUEST) as NODE " + " , ( SELECT AST.ALPHA_SUBTAB_DESCRIPTION "
					+ " FROM ALPHA_SUB_TAB AST  " + " WHERE AST.ALPHA_TAB = T3.ADF_SCHEDULE_STATUS_AT  "
			...
			strQueryAppr = new StringBuffer(" Select T1.COUNTRY, T1.LE_BOOK, "
					...
					+ " CASE WHEN (SELECT COUNT (1) FROM ADF_PROCESS_CONTROL APC WHERE APC.ADF_NUMBER = T1.ADF_NUMBER AND (APC.EOD_INITIATED_FLAG = 'Y' OR APC.AUTH_STATUS in (0,4) )) > 0 THEN 'Y' ELSE 'N' END AS EOD_INITIATED_FLAG, "
					+ " NVL(T3.NODE_OVERRIDE,T3.NODE_REQUEST) as NODE " + " , ( SELECT AST.ALPHA_SUBTAB_DESCRIPTION "
					+ " FROM ALPHA_SUB_TAB AST  " + " WHERE AST.ALPHA_TAB = T3.ADF_SCHEDULE_STATUS_AT  "
					+ " AND AST.ALPHA_SUB_TAB =  "
					+ " (CASE WHEN T3.XL_SPLIT_STATUS = 'Y' AND T3.ADF_SCHEDULE_STATUS = 'P' THEN 'I' ELSE T3.ADF_SCHEDULE_STATUS END)) ADF_SCHE_STATUS , NVL(T4.PROCESSED_FLAG,'N') PROCESSED_FLAG "
					+ " FROM ADF_PROCESS_CONTROL T1, ADF_UPLOAD_FILING T4, TEMPLATE_NAMES T2, ADF_SCHEDULES T3 "
	...
	public int getCountOfRunningTemplates(String adfNumber, int processNumber) {
		...
		if (processNumber == 1) {
			...
			if ("MSSQL".equalsIgnoreCase(databaseType)) {
				strQuery = new String(" SELECT COUNT(1)  " + " FROM ADF_SCHEDULES T1 " + " WHERE T1.ADF_NUMBER = ? "
						...
						+ " AND EXISTS ( SELECT 1 " + " FROM ADF_PROCESS_CONTROL T2 , ADF_UPLOAD_FILING T5 "
						...
						+ " AND T2.COUNTRY = T5.COUNTRY" + " AND T2.LE_BOOK = T5.LE_BOOK"
						+ " AND T2.BUSINESS_DATE = T5.BUSINESS_DATE" + " AND T2.TEMPLATE_NAME = T5.TEMPLATE_NAME) ");
			} else {
				strQuery = new String(" SELECT COUNT(1) " + " FROM ADF_SCHEDULES T1 " + " WHERE T1.ADF_NUMBER = ? "
						+ " AND T1.ADF_SCHEDULE_STATUS IN (SELECT VVN.RETURN_VALUE_CHAR FROM VISION_VARIABLES_NARROW VVN WHERE VVN.VARIABLE = 'NON_ADF_SCHEDULE_PICKUP_STATUS_LIST')"
						+ " AND EXISTS ( SELECT 1 " + " FROM ADF_PROCESS_CONTROL T2 , ADF_UPLOAD_FILING T5 "
...
//						vObject.setVersionNo(getMaxVersionNo(vObject.getFileName()));
					...
					if (vObject.getFilingStatus() != 1 && collTemp != null && !collTemp.isEmpty()) {
						 ...
						 * strErrorDesc="File Upload in Progress for Template ID : "+vObjectPersis.
						 * getTemplateName(); throw new
						 * RuntimeCustomException(getResultObject(Constants.WE_HAVE_ERROR_DESCRIPTION));
						 * }
						 */
	...
	protected int doUpdateAppr(UploadFilingVb vObject) {
		...
		Object[] args = { vObject.getFileName(), nodeName, vObject.getFilingStatusNt(), vObject.getFilingStatus(),
				vObject.getMaker(), vObject.getVerifier(), vObject.getUploadDate(), vObject.getCountry(), trimmedLeBook,
				vObject.getTemplateName(), vObject.getBusinessDate() };
		return getJdbcTemplate().update(query, args);
	}

	public int doUpdateScheduleStatus(UploadFilingVb vObject) {
		String query = "update ADF_SCHEDULES SET ADF_SCHEDULE_STATUS = ?, SUBMITTER_ID = '" + intCurrentUserId
				+ "', XL_SPLIT_STATUS = 'N' WHERE ADF_NUMBER = ? ";
	...
	public int doUpdateProcessStatus(UploadFilingVb vObject) {
		...
		Object[] args = { vObject.getAcquisitionStatus(), vObject.getCountry(), trimmedLeBook,
				vObject.getBusinessDate(), vObject.getTemplateName() };
		return getJdbcTemplate().update(query, args);
	}
	...
	protected int doInsertionAppr(UploadFilingVb vObject) {
		...
		if ("MSSQL".equalsIgnoreCase(databaseType)) {
			...
			query = "Insert Into ADF_UPLOAD_FILING ( COUNTRY, LE_BOOK, FILE_NAME, TEMPLATE_NAME, BUSINESS_DATE, UPLOAD_DATE, FILING_STATUS_NT, FILING_STATUS,"
					+ " MAKER, VERIFIER, INTERNAL_STATUS, CURR_VERSION_NO, DATE_LAST_MODIFIED, DATE_CREATION, NODE_REQUEST, PROCESSED_FLAG) "
					+ "Values (?, ?, ?, ?, to_char( ?, 'dd-Mon-yyyy')," + "sysdate"
					+ ", ?, ?, ?, ?, ?, ?, sysdate, sysdate, ?, ?)";
		}
	...
	public long getUploadStatus(UploadFilingVb vObject) {
		StringBuffer strBufApprove = new StringBuffer(
				"select UPLOAD_STATUS from VISION_UPLOAD where upload_sequence = '" + vObject.getUploadSequence()
						+ "' ");
		try {
	...
	public synchronized int doInsertionApprUpload(UploadFilingVb vObject) {
		...
		Object[] args = { vObject.getFeedStgName(), addXlsxExtension, vObject.getUploadSequence(),
				vObject.getVisionTableUploadDate(), vObject.getUploadStatusNt(), vObject.getUploadStatus(),
				vObject.getMaker() };
		return getJdbcTemplate().update(query, args);
	}
	...
	public long getMaxVersionNo(ADFAcqVersionsVb vObject) {
		...
		if ("MSSQL".equalsIgnoreCase(databaseType)) {
			strQuery = new String(
					"SELECT ISNULL(MAX(VERSION_NO), 0) FROM ACQ_VERSION_NO WHERE COUNTRY = ? AND LE_BOOK = ? AND "
							+ "BUSINESS_DATE = CONVERT(datetime, ?, 103) AND FEED_DATE = CONVERT(datetime, ?, 103) AND TEMPLATE_NAME = ? ");
		} else {
			strQuery = new String(
					"SELECT NVL(MAX(VERSION_NO), 0) FROM ACQ_VERSION_NO WHERE COUNTRY = ? AND LE_BOOK = ? AND "
	...
	public String getADFBuildStatus(String country, String leBook, String businessDate, String tempalteName) {
		...
		if ("MSSQL".equalsIgnoreCase(databaseType)) {
			strQuery = new String(
					"SELECT ISNULL(MAX(ADF_SCHEDULE_STATUS), 'C') ADF_SCHEDULE_STATUS FROM ADF_SCHEDULES WHERE ADF_NUMBER =("
							+ "SELECT MAX(ADF_NUMBER) FROM ADF_PROCESS_CONTROL WHERE COUNTRY = ? AND LE_BOOK = ? AND "
							+ "BUSINESS_DATE = CONVERT(datetime, ?, 103) )");
		} else {
			strQuery = new String(
					"SELECT NVL(MAX(ADF_SCHEDULE_STATUS), 'C') ADF_SCHEDULE_STATUS FROM ADF_SCHEDULES WHERE ADF_NUMBER =("
							+ "SELECT MAX(ADF_NUMBER) FROM ADF_PROCESS_CONTROL WHERE COUNTRY = ? AND LE_BOOK = ? AND "
	...
	public int getADFBuildStatusWith(String originalFileName) {
		...
		String strQuery = new String(
				"SELECT COUNT(1) FROM ADF_SCHEDULES T1 WHERE  T1.ADF_SCHEDULE_STATUS IN ('I','M') AND T1.ADF_NUMBER IN "
	...
	public List<String> getDynamicHashVariable(String variableName) {
		String strQuery = new String(
				"SELECT VARIABLE_SCRIPT, SCRIPT_TYPE FROM VISION_DYNAMIC_HASH_VAR WHERE VARIABLE_NAME = '"
						+ variableName + "' ");
	...
	public int insertUpdateAdfAcqVersion(ADFAcqVersionsVb vObject) {
		...
		/*
		 * String
		 * sql="select COUNT(1) VERSION_NO from ACQ_VERSION_NO where COUNTRY='"+vObject.
		 * getCountry()+"' AND LE_BOOK='"+trimmedLeBook+"' AND FEED_DATE=To_Date('"
		 * +vObject.getFeedDate()+"', 'dd-MMM-yyyy') AND TEMPLATE_NAME='"+vObject.
		 * getTemplateName()+"' AND BUSINESS_DATE=To_Date('"+vObject.getBusinessDate()
		 * +"', 'dd-MMM-yyyy')";
		 * 
		 * long count=getJdbcTemplate().queryForLong(sql);
		 * 
		 * if(count>0){ String query =
		 * "Select MAX(VERSION_NO) From ACQ_VERSION_NO where COUNTRY='"+vObject.
		 * getCountry()+"' " +
		 * "AND LE_BOOK='"+trimmedLeBook+"' AND FEED_DATE=To_Date('"+vObject.getFeedDate
		 * ()+"', 'dd-MMM-yyyy')" + " AND TEMPLATE_NAME='"+vObject.getTemplateName()+"'"
		 * +
		 * "AND BUSINESS_DATE=To_Date('"+vObject.getBusinessDate()+"', 'dd-MMM-yyyy')";
		 * try{ vObject.setVersionNo(getJdbcTemplate().queryForInt(query)); return 1;
		 * }catch(Exception ex){ ex.printStackTrace(); return 0; } }else{
		 */
		...
		Object args[] = { vObject.getCountry(), trimmedLeBook, vObject.getBusinessDate(), vObject.getFeedDate(),
				vObject.getTemplateName(), vObject.getAcquisitionProcessTypeAt(), vObject.getAcquisitionProcessType(),
				vObject.getVersionNo(), vObject.getProcessStartTime(), vObject.getProcessEndTime(),
				vObject.getExtStartTime(), vObject.getExtEndTime(), vObject.getUplStartTime(), vObject.getUplEndTime(),
				vObject.getIntStartTime(), vObject.getIntEndTime(), vObject.getAcqVersionStatusNt(),
				vObject.getAcqVersionStatus(), vObject.getRecordIndicatorNt(), vObject.getRecordIndicator(),
	...
	public int getCountOfUploadTables(UploadFilingVb dObj) {
		...
		;
		StringBuffer strBufApprove = new StringBuffer(
				"select count(1) from VISION_TABLES MAXTABLES, PROFILE_PRIVILEGES PP, VISION_USERS MU "
	...
	public List<UploadFilingVb> getDefaultTemplates(UploadFilingVb dObj) {
		...
		if ("MSSQL".equalsIgnoreCase(databaseType)) {
			strQueryAppr = new String(
					"select T1.COUNTRY, T1.LE_BOOK ,T1.TEMPLATE_NAME, (select LE_BOOK + '-' + LEB_DESCRIPTION from le_book where country = ? "
...
//					uploadFilingVb.setUploadDate(rs.getString("UPLOAD_DATE"));
	...
	public List<UploadFilingVb> getDataFromProcessControl(UploadFilingVb dObj) {
		...
		if ("MSSQL".equalsIgnoreCase(databaseType)) {
			strQueryAppr = new String(
					"select T1.COUNTRY, T1.LE_BOOK ,T1.TEMPLATE_NAME,Format(T1.BUSINESS_DATE, 'dd-MMM-yyyy') BUSINESS_DATE,T1.ACQUISITION_PROCESS_TYPE,"
							...
							+ " T1.TEMPLATE_NAME = T3. TEMPLATE_NAME" + " AND T1.COUNTRY = T2.COUNTRY "
							+ " AND T1.LE_BOOK = T2.LE_BOOK " + " AND T1.TEMPLATE_NAME = T2.TEMPLATE_NAME "
			...
			strQueryAppr = new String(
					"select T1.COUNTRY, T1.LE_BOOK ,T1.TEMPLATE_NAME,to_char(T1.BUSINESS_DATE, 'dd-Mon-yyyy') BUSINESS_DATE,T1.ACQUISITION_PROCESS_TYPE,"
...
//					uploadFilingVb.setUploadDate(rs.getString("UPLOAD_DATE"));
	...
	public List<TemplateStgVb> getTemplateStgData(UploadFilingVb uploadFilingVb) {
		...
		String query = new String("SELECT T1.REPORT_ID,T1.SESSION_ID,T1.TEMPLATE_NAME, "
				...
				+ " FROM TEMPLATES_STG T1,XBRL_CONFIGURATION T2 " + " WHERE T1.REPORT_ID = ?  "
				+ " AND T1.SESSION_ID= ? " + " AND T1.REPORT_ID = T2.TEMPLATE_NAME " + " AND T1.COL_ID = T2.COL_ID "
				+ " AND T1.CELL_DATA IS NOT NULL " + " ORDER BY Tab_ID, Row_Id,Col_Id,Sort_Field");
	...
	private int insetAuditTrailAdfProcessData(long auditTrailSequenceId, String description, String detailDesc,
			UploadFilingVb vObject) {
		String trimmedLeBook = removeDescLeBook(vObject.getLeBook());
		String query = "";
		if ("MSSQL".equalsIgnoreCase(databaseType)) {
			query = "Insert Into ADF_AUDIT_TRAIL (COUNTRY, LE_BOOK, BUSINESS_DATE, FEED_DATE, TEMPLATE_NAME, AUDIT_TRAIL_SEQUENCE_ID, "
					...
					+ " Values ('" + vObject.getCountry() + "', '" + trimmedLeBook + "', to_date('"
					+ vObject.getBusinessDate() + "', 'dd-MMM-yyyy'), " + " '" + vObject.getTemplateName() + "', '"
					+ auditTrailSequenceId + "', GetDate(), " + "'" + description + "', '" + detailDesc
					+ "', 1075, 'NA', " + "1007, 9999, 1008, 9999, GetDate(), GetDate() ) ";
		} else {
			query = "Insert Into ADF_AUDIT_TRAIL (COUNTRY, LE_BOOK, BUSINESS_DATE, FEED_DATE, TEMPLATE_NAME, AUDIT_TRAIL_SEQUENCE_ID, "
					+ " DATETIME_STAMP, AUDIT_DESCRIPTION, AUDIT_DESCRIPTION_DETAIL, TYPE_OF_ALERT_AT, TYPE_OF_ALERT, "
					+ " ALERT_TYPE_NT, ALERT_TYPE, ACTION_STATUS_NT, ACTION_STATUS, DATE_LAST_MODIFIED, DATE_CREATION)"
					+ " Values ('" + vObject.getCountry() + "', '" + trimmedLeBook + "', to_date('"
					+ vObject.getBusinessDate() + "', 'dd-Mon-yyyy'), " + " '" + vObject.getTemplateName() + "', '"
					+ auditTrailSequenceId + "', sysdate, " + "'" + description + "', '" + detailDesc
					+ "', 1075, 'NA', " + "1007, 9999, 1008, 9999, sysdate, sysdate ) ";
		}
		try {
	...
	private long getMaxSequence(UploadFilingVb vObject) {
		...
		if ("MSSQL".equalsIgnoreCase(databaseType)) {
			strQuery = new String(
					"SELECT ISNULL(MAX(AUDIT_TRAIL_SEQUENCE_ID), 1000) MAX_COUNT FROM ADF_AUDIT_TRAIL WHERE COUNTRY = '"
							+ vObject.getCountry() + "' AND LE_BOOK = '" + trimmedLeBook + "' "
							+ "AND BUSINESS_DATE = TO_DATE('" + vObject.getBusinessDate() + "', 'dd-MMM-yyyy') "
							+ "AND TEMPLATE_NAME = '" + vObject.getTemplateName() + "' ");
		} else {
			strQuery = new String(
					"SELECT NVL(MAX(AUDIT_TRAIL_SEQUENCE_ID), 1000) MAX_COUNT FROM ADF_AUDIT_TRAIL WHERE COUNTRY = '"
							+ vObject.getCountry() + "' AND LE_BOOK = '" + trimmedLeBook + "' "
	...
	public int updateStausOfEachProcess(UploadFilingVb vObject, String columnName, String value) {
		...
		String query = "";
		if ("MSSQL".equalsIgnoreCase(databaseType)) {
			query = "UPDATE ADF_PROCESS_CONTROL set " + columnName + " = '" + value + "' WHERE COUNTRY = '"
					+ vObject.getCountry() + "' AND LE_BOOK = '" + trimmedLeBook + "' "
					+ "AND BUSINESS_DATE = CONVERT(datetime, '" + vObject.getBusinessDate()
					+ "', 103) AND TEMPLATE_NAME = '" + vObject.getTemplateName() + "' ";
		} else {
			query = "UPDATE ADF_PROCESS_CONTROL set " + columnName + " = '" + value + "' WHERE COUNTRY = '"
					+ vObject.getCountry() + "' AND LE_BOOK = '" + trimmedLeBook + "' "
					+ "AND BUSINESS_DATE = to_date('" + vObject.getBusinessDate()
					+ "', 'dd-Mon-yyyy') AND TEMPLATE_NAME = '" + vObject.getTemplateName() + "' ";
		}
		try {
			if ("N".equalsIgnoreCase(value)) {
				if ("MSSQL".equalsIgnoreCase(databaseType)) {
					query = "UPDATE ADF_PROCESS_CONTROL set " + columnName + " = '" + value + "', NEXT_PROCESS_TIME = "
							...
							+ "FROM ADF_PROCESS_CONTROL WHERE COUNTRY = '" + vObject.getCountry() + "' AND LE_BOOK = '"
							+ trimmedLeBook + "' " + "AND BUSINESS_DATE = CONVERT(datetime, '"
							+ vObject.getBusinessDate() + "', 103)  AND TEMPLATE_NAME = '" + vObject.getTemplateName()
							+ "')  " + "WHERE COUNTRY = '" + vObject.getCountry() + "' AND LE_BOOK = '" + trimmedLeBook
					...
					query = "UPDATE ADF_PROCESS_CONTROL set " + columnName + " = '" + value + "', NEXT_PROCESS_TIME = "
							+ "(SELECT NEXT_PROCESS_TIME+((SELECT VALUE FROM VISION_VARIABLES WHERE VARIABLE =  'ADF_IDLE_TIME') /60)/1440 "
							+ "FROM ADF_PROCESS_CONTROL WHERE COUNTRY = '" + vObject.getCountry() + "' AND LE_BOOK = '"
							+ trimmedLeBook + "' " + "AND BUSINESS_DATE = to_date('" + vObject.getBusinessDate()
							+ "', 'dd-Mon-yyyy') AND TEMPLATE_NAME = '" + vObject.getTemplateName() + "')  "
							+ "WHERE COUNTRY = '" + vObject.getCountry() + "' AND LE_BOOK = '" + trimmedLeBook
							+ "' AND BUSINESS_DATE = to_date('" + vObject.getBusinessDate() + "', 'dd-Mon-yyyy') "
							+ "AND TEMPLATE_NAME = '" + vObject.getTemplateName() + "' ";
	...
	public int updateStausOfUploadFiling(UploadFilingVb vObject, String columnName, String value) {
		...
		String query = "";
		if ("MSSQL".equalsIgnoreCase(databaseType)) {
			query = "UPDATE ADF_UPLOAD_FILING set " + columnName + " = '" + value + "' WHERE COUNTRY = '"
					+ vObject.getCountry() + "' AND LE_BOOK = '" + trimmedLeBook + "' "
					+ "AND BUSINESS_DATE =  CONVERT(datetime, '" + vObject.getBusinessDate()
					+ "', 103)AND TEMPLATE_NAME = '" + vObject.getTemplateName() + "' ";
		} else {
	...
	public int updateStausOfVersionTable(UploadFilingVb vObject, String columnName, String columnValue) {
		...
		String query = "";
		if ("MSSQL".equalsIgnoreCase(databaseType)) {
			query = "UPDATE ACQ_VERSION_NO set " + columnName + " = CONVERT(datetime, '" + columnValue
					+ "', 103) WHERE COUNTRY = ? AND LE_BOOK = ? AND "
					+ "BUSINESS_DATE = CONVERT(datetime, ?, 103) AND FEED_DATE = CONVERT(datetime, ?, 103) AND TEMPLATE_NAME = ? AND VERSION_NO = ?  ";
		} else {
			query = "UPDATE ACQ_VERSION_NO set " + columnName + " = to_date('" + columnValue
					+ "', 'DD-MON-RRRR HH24:MI:SS')  WHERE COUNTRY = ? AND LE_BOOK = ? AND "
					+ "BUSINESS_DATE = to_date( ?, 'dd-Mon-yyyy') AND FEED_DATE = to_date( ?, 'dd-Mon-yyyy') AND TEMPLATE_NAME = ? AND VERSION_NO = ?  ";
		}
		Object args[] = { vObject.getCountry(), trimmedLeBook, vObject.getBusinessDate(), vObject.getFeedDate(),
				vObject.getTemplateName(), vObject.getVersionNo() };
		logger.info("Update Query : " + query + " Arguemnts : " + vObject.getCountry() + ", " + trimmedLeBook + ", "
				+ vObject.getBusinessDate() + ", " + vObject.getFeedDate() + ", " + vObject.getTemplateName() + ", "
				+ vObject.getVersionNo());
	...
	public String getSysteDateForAdfProcess() {
		String query = "";
		if ("MSSQL".equalsIgnoreCase(databaseType))
			return getJdbcTemplate().queryForObject("SELECT Format(GetDate(),'dd-MMM-yyy hh:MM:ss') SYSTEM_DAT  ",
					String.class);
		else
			return getJdbcTemplate().queryForObject("SELECT to_date(sysdate,'dd-Mon-yyy HH:mi:ss') SYSTEM_DAT  ",
					String.class);
	}
	...
	public ArrayList<String> getCountOfErrorsInDBP(UploadFilingVb vObject) {
		...
		if ("MSSQL".equalsIgnoreCase(databaseType)) {
			strQuery = new String(
					"SELECT COUNT(1) ERROR_COUNT, MIN(CASE WHEN T2.ERROR_TYPE IN (1,3) THEN 'W' WHEN T2.ERROR_TYPE IN (2) THEN 'S' ELSE 'N' END) ERROR_TYPE "
							+ "FROM VISION_DBP_ERRORS T1, ERROR_CODES T2 WHERE T1.ERROR_CODE = T2.ERROR_CODE AND COUNTRY = '"
							+ vObject.getCountry() + "' AND LE_BOOK = '" + trimmedLeBook + "' "
							+ "AND BUSINESS_DATE = CONVERT(datetime, '" + vObject.getBusinessDate() + "', 103)  "
							+ "AND FEED_DATE =  CONVERT(datetime, '" + vObject.getFeedDate()
							+ "', 103)   AND TEMPLATE_NAME = '" + vObject.getTemplateName() + "' ");
		} else {
			strQuery = new String(
					"SELECT COUNT(1) ERROR_COUNT, MIN(CASE WHEN T2.ERROR_TYPE IN (1,3) THEN 'W' WHEN T2.ERROR_TYPE IN (2) THEN 'S' ELSE 'N' END) ERROR_TYPE "
							...
							+ vObject.getCountry() + "' AND LE_BOOK = '" + trimmedLeBook + "' "
							+ "AND BUSINESS_DATE = to_date('" + vObject.getBusinessDate() + "', 'dd-Mon-yyyy') "
							+ "AND FEED_DATE = to_date('" + vObject.getFeedDate()
							+ "', 'dd-Mon-yyyy') AND TEMPLATE_NAME = '" + vObject.getTemplateName() + "' ");
		}
		return getJdbcTemplate().query(strQuery, new ResultSetExtractor<ArrayList<String>>() {
			public ArrayList<String> extractData(ResultSet rs)
	...
	public String replaceMacroValues(UploadFilingVb vObject, String adfAdFilePath, String adfLogPath,
			String adfFileUploadPath, String source) {
		String trimmedLeBook = removeDescLeBook(vObject.getLeBook()).toUpperCase();
		...
		if (procedureValue.indexOf("#DEFAULT_ACQ_PROCESS_TYPE#") > 0) {
			procedureValue = procedureValue.replaceAll("#DEFAULT_ACQ_PROCESS_TYPE#",
					vObject.getDefaultAcqProcessType());
		}
	...
	public int updateExtractionErrorCount(UploadFilingVb vObject) {
		...
		if ("MSSQL".equalsIgnoreCase(databaseType)) {
			...
			query = "UPDATE ADF_PROCESS_CONTROL SET  EXT_ITERATION_COUNT = (SELECT NVL(MAX(EXT_ITERATION_COUNT), 0)+1 FROM ADF_PROCESS_CONTROL "
					+ "WHERE COUNTRY = '" + vObject.getCountry() + "' AND LE_BOOK = '" + vObject.getLeBook()
					+ "' AND TEMPLATE_NAME = '" + vObject.getTemplateName() + "' AND BUSINESS_DATE = to_date('"
					+ vObject.getBusinessDate() + "', 'dd-Mon-yyyy'))  " + "WHERE COUNTRY = '" + vObject.getCountry()
					+ "' AND LE_BOOK = '" + vObject.getLeBook() + "' AND TEMPLATE_NAME = '" + vObject.getTemplateName()
					+ "' AND BUSINESS_DATE = to_date('" + vObject.getBusinessDate() + "', 'dd-Mon-yyyy')";
		}
		try {
			getJdbcTemplate().execute(query);
	...
	public List<UploadFilingVb> getIntegrityErrors(UploadFilingVb dObj) {
		...
		if ("MSSQL".equalsIgnoreCase(databaseType)) {
			strQueryAppr = new String(
					"SELECT COUNT(1), LE_BOOK, BUSINESS_DATE, TEMPLATE_NAME,  T1.ERROR_DESCRIPTION FROM VISION_DBP_ERRORS T1, ERROR_CODES T2 "
	...
	public List<ColumnHeadersVb> getColumnHeaders(String reportId, String sessionId) {
		...
		String query = new String(
				"SELECT REPORT_ID, SESSION_ID, LABEL_ROW_NUM, LABEL_COL_NUM, CAPTION, COLUMN_WIDTH, COL_TYPE, ROW_SPAN, COL_SPAN, NUMERIC_COLUMN_NO "
	...
	public String getListReportDataForExportAsXMLString(PromptTreeVb promptTree,
			final List<ColumnHeadersVb> columnHeaders, long min, long max) {
		String query = "";
		try {
			query = "SELECT T1.* FROM " + promptTree.getTableName() + " T1 WHERE SORT_FIELD <=" + max
					+ " AND SORT_FIELD >" + min + " ORDER BY SORT_FIELD";
			System.out.println("Query :" + query);
...
//					    	System.out.println("Col Name :"+columnHeader.getCaption());
							...
							if (columnHeader.getCaption().contains(",")) {
								...
								if (value == null)
									value = "";
								lResult.append("<").append(parts[1]).append(">")
										.append(StringEscapeUtils.escapeXml(value)).append("</").append(parts[1])
										.append(">");
							} else {
							...
							/*
							 * String value = rs.getString(columnHeader.getCaption()); if(value == null)
							 * value="";
							 * lResult.append("<").append(columnHeader.getCaption()).append(">").append(
							 * StringEscapeUtils.escapeXml(value)).append("</").append(columnHeader.
							 * getCaption()).append(">");
							 */
						}
						try {
							String HEADERTITLE_TEXTValue = rs.getString("HEADERTITLE_TEXT");
							if (HEADERTITLE_TEXTValue == null)
								HEADERTITLE_TEXTValue = "";
							lResult.append("<").append("headerText").append(">")
									.append(StringEscapeUtils.escapeXml(HEADERTITLE_TEXTValue)).append("</")
	...
	public PromptTreeVb callProcToPopulateListReportData(UploadFilingVb uploadFilingVb, String legalVehcile,
			int currentUserId) {
		setServiceDefaults();
	...
	public String getLegalVehicle(UploadFilingVb vObj) {
		...
		return getJdbcTemplate().queryForObject("SELECT LEGAL_VEHICLE FROM LE_BOOK WHERE COUNTRY = '"
				+ vObj.getCountry() + "' AND LE_BOOK = '" + trimmedLeBook.toUpperCase() + "' ", String.class);
	}
		...
		try {
			if ("MSSQL".equalsIgnoreCase(databaseType))
				return getJdbcTemplate().queryForObject(
						"select LEGAL_VEHICLE+' - '+ lv_DESCRIPTION  from legal_vehicles where LEGAL_VEHICLE = '"
								+ legalVehicle + "'  ",
						String.class);
			else
				return getJdbcTemplate().queryForObject(
	...
	public String getTemplateNameDesc(String legalVehicle) {
		try {
			return getJdbcTemplate().queryForObject(
					"SELECT GENERAL_DESCRIPTION FROM TEMPLATE_NAMES WHERE TEMPLATE_NAME = '" + legalVehicle + "'  ",
					String.class);
		} catch (Exception e) {
			return "";
		}
	...
	public String getMajorBuildDesc(String majorBuild) {
		try {
			...
			return getJdbcTemplate().queryForObject(
					"SELECT program_description FROM PROGRAMS  WHERE PROGRAM = '" + majorBuild + "'  ", String.class);
		} catch (Exception e) {
			return "";
		}
	}
	...
	public String getAlphaSubTabDescription(int alphaTab, String alphaSubTab) {
		if ("MSSQL".equalsIgnoreCase(databaseType))
			return getJdbcTemplate().queryForObject(
					"select ALPHA_SUB_TAB+' - '+ ALPHA_SUBTAB_DESCRIPTION from ALPHA_SUB_TAB where alpha_tab = "
							+ alphaTab + " and ALPHA_SUB_TAB = '" + alphaSubTab + "' ",
					String.class);
		else
			return getJdbcTemplate().queryForObject(
					"select ALPHA_SUB_TAB||' - '|| ALPHA_SUBTAB_DESCRIPTION from ALPHA_SUB_TAB where alpha_tab = "
	...
	public synchronized long getMaxVersionNo(String fileName) {
		...
		if ("MSSQL".equalsIgnoreCase(databaseType))
			strBufApprove = new StringBuffer(
					"Select ISNULL(MAX(CURR_VERSION_NO), 0) FROM ADF_UPLOAD_FILING WHERE FILE_NAME = '" + fileName
							+ "'");
		else
			strBufApprove = new StringBuffer(
					"Select NVL(MAX(CURR_VERSION_NO), 0) FROM ADF_UPLOAD_FILING WHERE FILE_NAME = '" + fileName + "'");
		try {
			return getJdbcTemplate().queryForObject(strBufApprove.toString(), long.class);
		} catch (Exception ex) {
			ex.printStackTrace();
	...
	public int doUpdateVersionNo(String fileName, long versionNO) {
		...
		if (retVal == 0) {
			query = "SELECT COUNT(1) FROM ADF_UPLOAD_FILING WHERE FILE_NAME = '" + fileName
					+ "' AND INTERNAL_STATUS = 1 ";
			int recordCount = getJdbcTemplate().queryForObject(query, int.class);
			if (recordCount > 0) {
				query = "Update ADF_UPLOAD_FILING SET INTERNAL_STATUS = 0 WHERE FILE_NAME = '" + fileName
						+ "' AND INTERNAL_STATUS = 1 ";
	...
	public String getListReportDataForExportAsXMLStringHeader(PromptTreeVb promptTree,
			final List<ColumnHeadersVb> columnHeaders, long min, long max) {
		String query = "";
		try {
	...
	public String getXBRLFileName(UploadFilingVb vObj) {
		...
		String query = "";
		if ("MSSQL".equalsIgnoreCase(databaseType))
			query = "SELECT FILE_PATTERN FROM ADF_PROCESS_CONTROL WHERE COUNTRY = '" + vObj.getCountry()
					+ "' AND LE_BOOK = '" + trimmedLeBook.toUpperCase() + "' " + "AND CONVERT(datetime, '"
					+ vObj.getBusinessDate() + "', 103) AND TEMPLATE_NAME = '" + vObj.getTemplateName() + "' ";
		else
			query = "SELECT FILE_PATTERN FROM ADF_PROCESS_CONTROL WHERE COUNTRY = '" + vObj.getCountry()
					+ "' AND LE_BOOK = '" + trimmedLeBook.toUpperCase() + "' " + "AND BUSINESS_DATE=to_date('"
					+ vObj.getBusinessDate() + "','dd-Mon-yyyy') AND TEMPLATE_NAME = '" + vObj.getTemplateName() + "' ";
...
//		int ocunt = StringUtils.countMatches(leBook, "-");
		...
		StringBuffer strQueryAppr = new StringBuffer("SELECT DISTINCT t3.ALPHA_SUBTAB_DESCRIPTION,t1.FREQUENCY_PROCESS "
				...
				+ "' AND t1.LE_BOOK IN (" + trimmedLeBook + ") " + "AND t3.ALPHA_TAB = t1.FREQUENCY_PROCESS_AT "
				+ "AND t3.ALPHA_SUB_TAB = t1.FREQUENCY_PROCESS " + "AND t1.ACQUISITION_PROCESS_TYPE = 'XLS'");
		try {
	...
	public String getNodeRequestByAdfNumber(String adfNumber) {
		String sql = "SELECT CASE WHEN NODE_OVERRIDE IS  NULL THEN NODE_REQUEST ELSE NODE_OVERRIDE END NODE FROM ADF_SCHEDULES WHERE ADF_NUMBER = "
				+ adfNumber + " ";
	...
	public List<UploadFilingVb> getBusnessDate(UploadFilingVb uploadFilingVb) {
		...
		if ("MSSQL".equalsIgnoreCase(databaseType)) {
			strQueryAppr = new StringBuffer("SELECT DISTINCT Format(t1.BUSINESS_DATE, 'dd-MMM-yyyy') BUSINESS_DATE "
					...
					+ "WHERE t1.ADF_NUMBER = t2.adf_number and t1.ACQUISITION_PROCESS_TYPE = 'XLS' and t1.COUNTRY = '"
					+ uploadFilingVb.getCountry() + "' and t1.LE_BOOK = '" + trimLeBook + "'"
					+ " AND T1.FREQUENCY_PROCESS='" + uploadFilingVb.getProcessFrequency() + "' AND T1.MAJOR_BUILD = '"
					+ uploadFilingVb.getMajorBuild() + "'");
		} else {
	...
	public List<UploadFilingVb> getQueryPopupResults(UploadFilingVb dObj) {
		...
		try {
			/*
			 * //String queryString=""; if (ValidationUtil.isValid(dObj.getLegalVehicle()))
			 ...
			 * strBufApprove.append(" AND T1.COUNTRY LIKE '"+dObj.getCountry()
			 * +"'  AND T3.LEGAL_VEHICLE IN(SELECT LEGAL_VEHICLE FROM LEGAL_VEHICLES ");
			 * strBufApprove.
			 * append(" START WITH UPPER(LEGAL_VEHICLE) = ?  CONNECT BY PRIOR LEGAL_VEHICLE = PARENT_LV  AND LEGAL_VEHICLE != PARENT_LV)"
			...
			if (ValidationUtil.isValid(SessionContextHolder.getContext().getUpdateRestrictionLeBook())) {
				if ("MSSQL".equalsIgnoreCase(databaseType))
					strBufApprove.append("AND T1.COUNTRY+ '-' +T1.LE_BOOK IN("
							+ SessionContextHolder.getContext().getUpdateRestrictionLeBook().toUpperCase() + ") ");
				else
					strBufApprove.append("AND T1.COUNTRY|| '-' ||T1.LE_BOOK IN("
							+ SessionContextHolder.getContext().getUpdateRestrictionLeBook().toUpperCase() + ") ");
			...
			if (ValidationUtil.isValid(dObj.getBusinessDate()) && !"-1".equalsIgnoreCase(dObj.getBusinessDate())) {
				...
				// CommonUtils.addToQuery("T1.BUSINESS_DATE = Format(?, 'dd-MMM-yyyy')",
				// strBufApprove);
			}
		} catch (Exception ex) {
			ex.printStackTrace();
	...
	public List<UploadFilingVb> getCommentDrilldownResults(UploadFilingVb vObj, String tableName)
...
//			int ocunt = StringUtils.countMatches(leBook, "-");
		...
		StringBuffer strQueryAppr = new StringBuffer(
				"SELECT DISTINCT T1.MAJOR_BUILD " + " FROM ADF_SCHEDULES t1, ADF_PROCESS_CONTROL t2,ALPHA_SUB_TAB t3 "
						+ " WHERE t1.ADF_NUMBER = t2.adf_number and t1.COUNTRY = '" + uploadFilingVb.getCountry()
						+ "' AND t1.LE_BOOK IN (" + trimmedLeBook + ")" + " AND t3.ALPHA_TAB = t1.FREQUENCY_PROCESS_AT "
						+ " AND t3.ALPHA_SUB_TAB = t1.FREQUENCY_PROCESS " + " AND t1.ACQUISITION_PROCESS_TYPE = 'XLS' "
	...
	public ExceptionCode getPanelInfo1(UploadFilingVb dObj) {
		...
		try {
			/*
			 * StringBuffer strQueryAppr = new
			 * StringBuffer("SELECT DISTINCT T1.country,                                "+
			 * "                 T1.le_book,                                                                    "
			 * +
			 * "                 T1.le_book + ' - '                                                             "
			 * +
			...
			if ("MSSQL".equalsIgnoreCase(databaseType)) {
				strQueryAppr = new StringBuffer("SELECT T1.COUNTRY, " + "        T1.LE_BOOK, "
						...
						+ "        T1.FREQUENCY_PROCESS, " + "        (SELECT AST.ALPHA_SUBTAB_DESCRIPTION "
						+ "           FROM ALPHA_SUB_TAB AST "
						+ "          WHERE     AST.ALPHA_TAB = T1.FREQUENCY_PROCESS_AT "
						...
						+ "       AND T1.LE_BOOK = LEB.LE_BOOK " + "       AND T2.ACQUISITION_PROCESS_TYPE = 'XLS'  ");
			} else {
				strQueryAppr = new StringBuffer("SELECT T1.COUNTRY, " + "        T1.LE_BOOK, "
						+ "        T1.LE_BOOK || ' - ' || LEB.LEB_DESCRIPTION AS STAKEHOLDER, "
						+ "        T1.FREQUENCY_PROCESS, " + "        (SELECT AST.ALPHA_SUBTAB_DESCRIPTION "
			...
			String groupOrderBy = "";
			if ("MSSQL".equalsIgnoreCase(databaseType)) {
				groupOrderBy = " GROUP BY " + "        T1.COUNTRY, " + "        T1.LE_BOOK, "
						+ "        T1.LE_BOOK + ' - ' + LEB.LEB_DESCRIPTION, " + "        T1.FREQUENCY_PROCESS_AT, "
						+ "        T1.FREQUENCY_PROCESS, " + "        T1.BUSINESS_DATE, "
						+ "CASE WHEN T1.FREQUENCY_PROCESS = 'DLY' THEN FORMAT (T1.BUSINESS_DATE, 'dd-MMM-yyyy') "
						+ "            WHEN T1.FREQUENCY_PROCESS = 'MTH' THEN FORMAT (T1.BUSINESS_DATE, 'MMM-yyyy') "
						+ "            WHEN T1.FREQUENCY_PROCESS = 'QTR' THEN FORMAT (T1.BUSINESS_DATE, 'MMM-yyyy') "
						+ "            WHEN T1.FREQUENCY_PROCESS = 'ANN' THEN FORMAT (T1.BUSINESS_DATE, 'yyyy') "
						+ "       ELSE FORMAT (T1.BUSINESS_DATE, 'dd-MMM-yyyy') END " + " Order By  "
						+ "        T1.COUNTRY, " + "        T1.LE_BOOK, " + "        T1.FREQUENCY_PROCESS, "
						+ "        T1.BUSINESS_DATE ";
			} else {
				groupOrderBy = " GROUP BY " + "        T1.COUNTRY, " + "        T1.LE_BOOK, "
						+ "        T1.LE_BOOK || ' - ' || LEB.LEB_DESCRIPTION, " + "        T1.FREQUENCY_PROCESS_AT, "
						...
						+ "            WHEN T1.FREQUENCY_PROCESS = 'QTR' THEN to_char (T1.BUSINESS_DATE, 'Mon-yyyy') "
						+ "            WHEN T1.FREQUENCY_PROCESS = 'ANN' THEN to_char (T1.BUSINESS_DATE, 'yyyy') "
						+ "       ELSE to_char (T1.BUSINESS_DATE, 'dd-Mon-yyyy') END " + " Order By  "
						+ "        T1.COUNTRY, " + "        T1.LE_BOOK, " + "        T1.FREQUENCY_PROCESS, "
						+ "        T1.BUSINESS_DATE ";
			}
			strQueryAppr.append(groupOrderBy);
			if (SessionContextHolder.getContext().getVisionId() == 1232
					|| SessionContextHolder.getContext().getVisionId() == 1084) {
	...
	public ExceptionCode getPanelInfo2(UploadFilingVb dObj) {
		...
		try {
			 ...
			 * "  ADF_SCHEDULE_STATUS, "+ "  Format(T1.business_date, 'dd-MMM-yyyy') "+
			 * "  BUSINESS_DATE, "+
			...
			if ("MSSQL".equalsIgnoreCase(databaseType)) {
				strQueryAppr = new StringBuffer(" SELECT  " + "        T1.COUNTRY, " + "        T1.LE_BOOK, "
						+ "        T1.LE_BOOK + ' - ' + LEB.LEB_DESCRIPTION AS STAKEHOLDER, "
						+ "        T2.MAJOR_BUILD,  " + "        T1.FREQUENCY_PROCESS, " + "        T2.MAJOR_BUILD, "
						+ "        T6.PROGRAM_DESCRIPTION MAJOR_BUILD_DESC, "
						+ "        (SELECT AST.ALPHA_SUBTAB_DESCRIPTION " + "           FROM ALPHA_SUB_TAB AST "
						+ "          WHERE     AST.ALPHA_TAB = T1.FREQUENCY_PROCESS_AT "
						+ "                AND AST.ALPHA_SUB_TAB = T1.FREQUENCY_PROCESS) "
						+ "           AS FREQUENCY_PROCESS_DESC, " + "        (SELECT AST.ALPHA_SUBTAB_DESCRIPTION "
						+ "           FROM ALPHA_SUB_TAB AST "
						+ "          WHERE     AST.ALPHA_TAB = T2.ADF_SCHEDULE_STATUS_AT "
						+ "                AND AST.ALPHA_SUB_TAB = T2.ADF_SCHEDULE_STATUS) "
						+ "           ADF_SCHEDULE_STATUS, "
						...
						+
						// " sum(case when T1.ACQUISITION_STATUS != 'COMPNSERR' And
						// (T1.ACQUISITION_STATUS in ('COMPSERR') OR T1.ACQUISITION_STATUS like '%ERR')
						// then 1 else 0 end) ERRORED, "
						...
						+ "   FROM ADF_PROCESS_CONTROL T1, " + "        ADF_SCHEDULES T2, " + "        LE_BOOK LEB, "
						+ "        PROGRAMS T6 " + "  WHERE     T1.ADF_NUMBER = T2.ADF_NUMBER "
...
//							"	   sum(case when T1.ACQUISITION_STATUS != 'COMPNSERR' And (T1.ACQUISITION_STATUS in ('COMPSERR') OR T1.ACQUISITION_STATUS like '%ERR') then 1 else 0 end) ERRORED, "
			...
			if (ValidationUtil.isValid(dObj.getBusinessDate())) {
				...
				if ("MSSQL".equalsIgnoreCase(databaseType))
					strQueryAppr
							.append(" AND T1.BUSINESS_DATE = CONVERT(date, '" + dObj.getBusinessDate() + "', 103) ");
				else
					strQueryAppr.append(
							" AND T1.BUSINESS_DATE = to_date('" + dObj.getBusinessDate() + "', 'dd-Mon-yyyy')  ");
			}
			if (ValidationUtil.isValid(SessionContextHolder.getContext().getUpdateRestrictionLeBook())) {
				if ("MSSQL".equalsIgnoreCase(databaseType))
					strQueryAppr.append("and T1.COUNTRY+ '-' +T1.LE_BOOK IN ( "
			...
			String orderGroupBy = "";
			if ("MSSQL".equalsIgnoreCase(databaseType)) {
				orderGroupBy = " GROUP BY  " + "        T1.COUNTRY, " + "        T1.LE_BOOK, "
						+ "        T1.LE_BOOK + ' - ' + LEB.LEB_DESCRIPTION, " + "        T1.FREQUENCY_PROCESS, "
						+ "        T2.MAJOR_BUILD, " + "        T1.FREQUENCY_PROCESS_AT, "
						+ "        T1.FREQUENCY_PROCESS, " + "        T2.ADF_SCHEDULE_STATUS_AT, "
						+ "        T2.ADF_SCHEDULE_STATUS, " + "        T1.BUSINESS_DATE,"
						+ "		 T6.PROGRAM_DESCRIPTION  " + " Order By "
	...
	public ExceptionCode getPanelInfo3(UploadFilingVb dObj) {
		...
		try {
			...
			if ("MSSQL".equalsIgnoreCase(databaseType)) {
				strQueryAppr = new StringBuffer(" SELECT " + "  T1.COUNTRY, " + "  T1.LE_BOOK, "
						+ "  ISNULL (T2.NODE_OVERRIDE, T2.NODE_REQUEST) AS NODE, " + "  TAPPR.FILE_NAME, "
						+ "  T2.ADF_SCHEDULE_STATUS, " + "  T2.MAJOR_BUILD, " +
//						"  FORMAT(TAPPR.UPLOAD_DATE, 'dd-MMM-yyyy') FEED_DATE , "+
						...
						+ "  T1.ACQUISITION_STATUS, " + "  (SELECT AST.ALPHA_SUBTAB_DESCRIPTION  "
						+ "   FROM ALPHA_SUB_TAB AST  " + "   WHERE AST.ALPHA_TAB = T1.ACQUISITION_STATUS_AT "
						+ "   AND AST.ALPHA_SUB_TAB = T1.ACQUISITION_STATUS) ACQUISITION_STATUS_DESC, "
						+ "  T1.EXCEL_TEMPLATE_ID, " + "  T1.TEMPLATE_NAME TEMPLATE_NAME_DESC,  " + "  T2.ADF_NUMBER, "
						+ "  T1.LE_BOOK + ' - ' + LEB.LEB_DESCRIPTION AS STAKEHOLDER, "
						+ "  FORMAT(T1.BUSINESS_DATE, 'dd-MMM-yyyy') BUSINESS_DATE, "
						...
						+ "  CONVERT(char(8), " + "  dateadd(second,DATEDIFF( " + " SECOND,V1.PROCESS_START_TIME, "
						...
						+ "  ,8) DURATION " + " FROM ADF_PROCESS_CONTROL T1 " + " INNER JOIN  ADF_SCHEDULES T2 "
						+ "   ON (T1.ADF_NUMBER = T2.ADF_NUMBER) " + " INNER JOIN TEMPLATE_NAMES T3 "
						+ "   ON (T1.TEMPLATE_NAME = T3.TEMPLATE_NAME) " + " INNER JOIN LE_BOOK LEB "
						+ "   ON (T1.COUNTRY = LEB.COUNTRY AND T1.LE_BOOK = LEB.LE_BOOK) "
						+ " LEFT OUTER JOIN ADF_UPLOAD_FILING TAPPR " + "   ON (TAPPR.COUNTRY = T1.COUNTRY "
						+ "   AND TAPPR.LE_BOOK = T1.LE_BOOK " + "   AND TAPPR.BUSINESS_DATE = T1.BUSINESS_DATE "
						+ "   AND TAPPR.TEMPLATE_NAME = T1.TEMPLATE_NAME) " + " LEFT OUTER JOIN ACQ_VERSION_NO V1 "
						+ "   ON (V1.COUNTRY = T1.COUNTRY " + "   AND V1.LE_BOOK = T1.LE_BOOK "
						+ "   AND V1.BUSINESS_DATE = T1.BUSINESS_DATE " + "   AND V1.FEED_DATE = T2.FEED_DATE "
						...
						+ "                          AND V1.TEMPLATE_NAME = V2.TEMPLATE_NAME)) " + " WHERE ");
			} else {
				strQueryAppr = new StringBuffer(" SELECT " + "  T1.COUNTRY, " + "  T1.LE_BOOK, "
						+ "  NVL (T2.NODE_OVERRIDE, T2.NODE_REQUEST) AS NODE, " + "  TAPPR.FILE_NAME, "
						+ "  T2.ADF_SCHEDULE_STATUS, " + "  T2.MAJOR_BUILD, " +
//							"  to_char(TAPPR.UPLOAD_DATE, 'dd-Mon-yyyy') FEED_DATE , "+
						...
						+
						/*
						 * "  to_char( "+ "  ((to_date(V1.PROCESS_START_TIME) -  "+
						...
						+ "FROM ADF_PROCESS_CONTROL T1 " + " INNER JOIN  ADF_SCHEDULES T2 "
						+ "   ON (T1.ADF_NUMBER = T2.ADF_NUMBER) " + " INNER JOIN TEMPLATE_NAMES T3 "
						+ "   ON (T1.TEMPLATE_NAME = T3.TEMPLATE_NAME) " + " INNER JOIN LE_BOOK LEB "
						+ "   ON (T1.COUNTRY = LEB.COUNTRY AND T1.LE_BOOK = LEB.LE_BOOK) "
						+ " LEFT OUTER JOIN ADF_UPLOAD_FILING TAPPR " + "   ON (TAPPR.COUNTRY = T1.COUNTRY "
			...
			params.add(dObj.getBusinessDate());
			String orderBy = "";
			// List<UploadFilingVb> resultlst = getQueryPopupResults(dObj,new
			// StringBuffer(), strQueryAppr, new String(), "", params, Panel3Mapper());
			...
			// exceptionCode =
			// commonApiDao.getCommonResultDataQuery(strQueryAppr.toString());
		} catch (Exception ex) {
			ex.printStackTrace();
