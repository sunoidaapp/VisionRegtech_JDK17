...
//	}
	...
	public int updateProcessControlheader(String processStatus, String requestNo, String cbStatus,
			TemplateScheduleVb dObj) {
		setServiceDefaults();
		...
		Object[] args = { processStatus, requestNo, cbStatus, dObj.getCountry(), dObj.getLeBook(), dObj.getTemplateId(),
				dObj.getReportingDate(), dObj.getSubmissionDate() };
		return getJdbcTemplate().update(query, args);
	}

	public int updateProcessControlheader1(String processStatus, String requestNo, String cbStatus,
			TemplateScheduleVb dObj) {
	...
	public List<TemplateConfigVb> getTemplateConfigDetail(TemplateConfigVb templateConfigVb) {
		...
		try {
			String query = "SELECT T1.COUNTRY,T1.LE_BOOK,T1.TEMPLATE_ID,T1.TEMPLATE_DESCRIPTION, UPPER(T1.SOURCE_TABLE) SOURCE_TABLE,T1.SOURCE_TYPE,T1.TYPE_OF_SUBMISSION,T2.RG_PROCESS_ID,T1.CSV_DELIMITER,T1.DATALIST, "
					...
					+ "					      AND T1.LE_BOOK = T2.LE_BOOK                                    "
					+ "					      AND T1.TEMPLATE_id = T2.TEMPLATE_id                        "
					+ "					      AND T2.RG_PROCESS_STATUS = 'SI' " + " AND T1.COUNTRY = ? "
					+ " AND T1.LE_BOOK= ? " + " AND T1.TEMPLATE_ID = ? " + " AND REPORTING_DATE =  " + dateConvert;

			Object[] args = { templateConfigVb.getCountry(), templateConfigVb.getLeBook(),
					templateConfigVb.getTemplateId(), templateConfigVb.getReportingDate() };
...
//			System.out.println("CSV data (without delimiter) has been successfully written to " + fileName);
	...
	public int updateProcessControl(String processStatus, TemplateScheduleVb dObj, boolean flag) {
		...
		query.append(" WHERE COUNTRY = ? AND LE_BOOK = ? AND TEMPLATE_ID = ? AND REPORTING_DATE = " + dateConvert
				+ " AND SUBMISSION_DATE = " + dateConvert);
	...
	public int updateProcessControl1(String processStatus, TemplateScheduleVb dObj, boolean flag) {
		...
		query.append("UPDATE RG_PROCESS_CONTROLS SET RG_PROCESS_STATUS = ?, RECORD_INDICATOR = ?,DATE_LAST_MODIFIED = "
				+ systemDate);
	...
	public int updateTemplatesHeader(String processStatus, TemplateScheduleVb dObj) {
		...
		StringBuilder query = new StringBuilder();
		query.append(
				"UPDATE RG_TEMPLATES_HEADER SET STATUS = ?, RECORD_INDICATOR = ?,DATE_LAST_MODIFIED=" + systemDate);
	...
	public List<TemplateConfigVb> getQueryResults(TemplateConfigVb dObj) {
		...
		strQueryAppr = new String(" SELECT TAPPR.COUNTRY,                        "
				...
				+ " DATE_CREATION,TAPPR.DATALIST ,TAPPR.CBK_FILE_NAME"
...
//				vObject.setProcessFrequencyDesc(rs.getString("PROCESS_FREQUENCY_DESC"));
	...
	public List<TemplateMappingVb> getQueryMappingList(TemplateMappingVb dObj) {
		...
		String orderBy = "";
		strBufApprove = new StringBuffer("SELECT" + " TAPPR.COUNTRY," + "TAPPR.LE_BOOK," + "TAPPR.TEMPLATE_ID,"
				+ "TAPPR.SOURCE_COLUMN," + "TAPPR.TARGET_COLUMN," + "TAPPR.TEMPLATE_STATUS," + "TAPPR.RECORD_INDICATOR,"
				+ "TAPPR.MAKER," +
...
//				vObject.setMappingStatusDesc(rs.getString("MAPPING_STATUS_DESC"));
	...
	public int insertAuditTrailSubmitData(TemplateConfigVb vObject, int auditSequence, String description,
			String detailDesc) {
		String query = "";
		...
		query = "INSERT INTO RG_SUBMIT_AUDIT_TRAIL (COUNTRY, LE_BOOK, REPORTING_DATE,submission_date, TEMPLATE_ID, AUDIT_TRAIL_SEQUENCE_ID, "
				+ "DATETIME_STAMP, AUDIT_DESCRIPTION, AUDIT_DESCRIPTION_DETAIL) VALUES ( ?,?," + dateConvert + " , "
				+ dateConvert + ",?, ?," + getDbFunction("SYSDATE") + ", ?,?)";
		Object objParams[] = { vObject.getCountry(), vObject.getLeBook(), vObject.getReportingDate(),
				vObject.getSubmissionDate(), vObject.getTemplateId(), auditSequence, description, detailDesc };
		try {
			getJdbcTemplate().update(query, objParams);
			return Constants.SUCCESSFUL_OPERATION;
		} catch (Exception e) {
			e.printStackTrace();
			logger.info("Error while inserting the Rg_Submit_Audit_Trail");
	...
	public int getMaxSequence(TemplateConfigVb vObject) {
		...
		strQuery = new String("SELECT " + nullFun
				+ "(MAX(AUDIT_TRAIL_SEQUENCE_ID), 0) MAX_COUNT FROM RG_SUBMIT_AUDIT_TRAIL WHERE"
				+ " COUNTRY = ? AND LE_BOOK = ?" + " AND REPORTING_DATE = " + dateConvert + " AND TEMPLATE_ID = ?");
		Object objParams[] = { vObject.getCountry(), vObject.getLeBook(), vObject.getReportingDate(),
		...
		try {
			...
			ex.printStackTrace();
			logger.info("Error while getting getMaxSequence");
			return count;
		}
	}
	...
	public String getRequestId(TemplateConfigVb vObject) {
		...
		query.append(
				"WHERE COUNTRY = ? AND LE_BOOK = ? AND TEMPLATE_ID = ? AND REPORTING_DATE = ? AND SUBMISSION_DATE = ?");
		List<Object> args = new ArrayList<>();
