...
public class RgSchedulerDao extends AbstractDao<CommonVb> {
	...
	public int updateRgBuildSchedules(RgBuildsVb vObject, String status) {
		...
		try {
			...
			String query = "UPDATE RG_BUILD_SCHEDULES SET " + updateCondition
					+ ",BUILD_SCHEDULE_STATUS = ? WHERE COUNTRY = ? AND LE_BOOK = ? "
					+ " AND BUILD_ID = ? AND SEQUENCE = ? ";

			Object args[] = { status, vObject.getCountry(), vObject.getLeBook(), vObject.getBuildId(),
					vObject.getSequence() };
	...
	public int updateLastRunStatus(RgBuildsVb vObject, String status) {
		...
		try {
			String query = "UPDATE RG_PROGRAMS SET LAST_RUN_STATUS = ?,LAST_RUN_DATE = SYSDATE,LAST_EXECUTED_BY = ? "
					+ " WHERE COUNTRY = ? AND LE_BOOK = ? " + " AND PROGRAM = ? ";
	...
	public int moveScheduleToHistory(RgBuildsVb vObject) {
		int retVal = 0;
		try {
			String query = " INSERT INTO RG_BUILD_SCHEDULES_HIS " + " SELECT * FROM RG_BUILD_SCHEDULES "
					+ " WHERE COUNTRY = ? AND LE_BOOK = ?  " + "  AND BUILD_ID  = ? AND SEQUENCE = ? ";

			Object args[] = { vObject.getCountry(), vObject.getLeBook(), vObject.getBuildId(), vObject.getSequence() };
			retVal = getJdbcTemplate().update(query, args);

			query = "DELETE FROM RG_BUILD_SCHEDULES " + " WHERE COUNTRY = ? AND LE_BOOK = ?  "
					+ "  AND BUILD_ID  = ? AND SEQUENCE = ? ";
			retVal = getJdbcTemplate().update(query, args);
		} catch (Exception e) {
			logger.error(e.getMessage());
	...
	public int deleteScheduleHeader(RgBuildsVb vObject) {
		int retVal = 0;
		try {
			String query = "DELETE FROM RG_BUILD_SCHEDULES_HEADER " + " WHERE COUNTRY = ? AND LE_BOOK = ?  "
					+ " AND BUILD_ID = ?  AND SEQUENCE = ? ";
