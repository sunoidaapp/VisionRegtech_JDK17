...
public class CustomerManualDao extends AbstractDao<CustomerManualColVb> {
	...
	public List<CustomerManualColVb> getQueryPopupResults1(CustomerManualColVb dObj) {
		...
		StringBuffer strBufApprove = new StringBuffer(
				"select * from (" + "SELECT COUNTRY, LE_BOOK, CUSTOMER_ID, COLUMN_NAME, COLUMN_VALUE,\r\n"
	...
	public List<CustomerManualColVb> getQueryPopupResults(CustomerManualColVb dObj) {
		...
		try {
			...
			return getQueryPopupResults(dObj, new StringBuffer(SQL_PENDING), new StringBuffer(SQL_APPROVE),
					WHERE_NOT_EXISTS, ORDER_BY, params, getMapper());
		} catch (Exception ex) {
			ex.printStackTrace();
	...
	public int getMaxSeq(CustomerManualColVb vObject) {
		...
		Object[] args = { vObject.getCountry(), vObject.getLeBook(), vObject.getCustomerId(), vObject.getColumnName() };
	...
	public int insertHistory(List<CustomerManualColVb> rows, String opTag) {
		...
		int total = 0;
		for (CustomerManualColVb r : rows) {
			// If you want to record opTag, add a column in HIS or ignore it. We just insert
			// the snapshot.
