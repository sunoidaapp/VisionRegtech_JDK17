...
public class ProductLineConfigDao extends AbstractDao<TransLineHeaderVb> {
	...
	protected RowMapper getMapper(){
		RowMapper mapper = new RowMapper() {
			public Object mapRow(ResultSet rs, int rowNum) throws SQLException {
				...
				vObject.setTransLineSubTypeDesc(rs.getString("TRANS_LINE_PROD_SUB_TYPE_DESC"));
				vObject.setTransLineGrp(rs.getString("TRANS_LINE_PROD_GRP"));
				vObject.setTransLineGrpDesc(rs.getString("TRANS_LINE_PROD_GRP_DESC"));
	...
	protected void setServiceDefaults() {
		serviceName = "ProductLineConfig";
		serviceDesc = "Product Line Config";
	...
	public List<TransLineHeaderVb> getQueryPopupResults(TransLineHeaderVb dObj) {
		...
		strBufApprove = new StringBuffer("Select * from ( SELECT TAppr.COUNTRY, TAppr.LE_BOOK, TAppr.TRANS_LINE_ID,TAppr.TRANS_LINE_DESCRIPTION, "+  
				" TAppr.TRANS_LINE_PROD_SUB_TYPE, "+
				" (select T1.ALPHA_SUBTAB_DESCRIPTION from ALPHA_SUB_TAB T1 where t1.Alpha_tab = TAppr.TRANS_LINE_PROD_SUB_TYPE_AT and T1.ALPHA_SUB_TAB=TAppr.TRANS_LINE_PROD_SUB_TYPE) TRANS_LINE_PROD_SUB_TYPE_DESC, "+                                 
				" TAppr.TRANS_LINE_PROD_GRP,  "+
				" (select T2.ALPHA_SUBTAB_DESCRIPTION from ALPHA_SUB_TAB T2 where t2.Alpha_tab = TAppr.TRANS_LINE_PROD_GRP_AT and T2.ALPHA_SUB_TAB=TAppr.TRANS_LINE_PROD_GRP) TRANS_LINE_PROD_GRP_DESC, "+
				" TAppr.EXTRACTION_FREQUENCY, "+
				" (select T5.ALPHA_SUBTAB_DESCRIPTION from ALPHA_SUB_TAB T5 where t5.Alpha_tab = TAppr.EXTRACTION_FREQUENCY_AT and T5.ALPHA_SUB_TAB=TAppr.EXTRACTION_FREQUENCY)EXTRACTION_FREQUENCY_DESC, "+
				...
				" TAppr. MAKER,  "+
				" (SELECT MIN(USER_NAME) FROM VISION_USERS WHERE VISION_ID = "+commonDao.getDbFunction("NVL")+"(TAppr.MAKER,0) ) MAKER_NAME, "+               
				...
				" FROM RA_MST_TRANS_LINE_HEADER TAppr,NUM_SUB_TAB T3,NUM_SUB_TAB T4 "+
				" Where TAppr.Trans_Line_Type = 'P'  "+
				" and t3.NUM_tab = TAppr.TRANS_LINE_STATUS_NT "+                                                                               
		...
		strBufPending = new StringBuffer("Select * from ( SELECT TPend.COUNTRY, TPend.LE_BOOK, TPend.TRANS_LINE_ID,TPend.TRANS_LINE_DESCRIPTION, "+  
				" TPend.TRANS_LINE_PROD_SUB_TYPE, "+
				" (select T1.ALPHA_SUBTAB_DESCRIPTION from ALPHA_SUB_TAB T1 where t1.Alpha_tab = TPend.TRANS_LINE_PROD_SUB_TYPE_AT and T1.ALPHA_SUB_TAB=TPend.TRANS_LINE_PROD_SUB_TYPE) TRANS_LINE_PROD_SUB_TYPE_DESC, "+                                 
				" TPend.TRANS_LINE_PROD_GRP,  "+
				" (select T2.ALPHA_SUBTAB_DESCRIPTION from ALPHA_SUB_TAB T2 where t2.Alpha_tab = TPend.TRANS_LINE_PROD_GRP_AT and T2.ALPHA_SUB_TAB=TPend.TRANS_LINE_PROD_GRP) TRANS_LINE_PROD_GRP_DESC, "+
				" TPend.EXTRACTION_FREQUENCY, "+
				" (select T5.ALPHA_SUBTAB_DESCRIPTION from ALPHA_SUB_TAB T5 where t5.Alpha_tab = TPend.EXTRACTION_FREQUENCY_AT and T5.ALPHA_SUB_TAB=TPend.EXTRACTION_FREQUENCY)EXTRACTION_FREQUENCY_DESC, "+
				...
				" TPend. MAKER, "+
				" (SELECT MIN(USER_NAME) FROM VISION_USERS WHERE VISION_ID = "+commonDao.getDbFunction("NVL")+"(TPend.MAKER,0) ) MAKER_NAME, "+               
				...
				" FROM RA_MST_TRANS_LINE_HEADER_PEND TPend,NUM_SUB_TAB T3,NUM_SUB_TAB T4 "+
				" Where TPend.Trans_Line_Type = 'P'  "+
				" and t3.NUM_tab = TPend.TRANS_LINE_STATUS_NT "+                                                                               
		...
		try
		{
			...
			if (dObj.getSmartSearchOpt() != null && dObj.getSmartSearchOpt().size() > 0) {
				...
				for (SmartSearchVb data: dObj.getSmartSearchOpt()){
					...
					case "transLineProdGrpDesc":
						CommonUtils.addToQuerySearch(" upper(TAPPR.TRANS_LINE_PROD_GRP_DESC) "+ val, strBufApprove, data.getJoinType());
						CommonUtils.addToQuerySearch(" upper(TPend.TRANS_LINE_PROD_GRP_DESC) "+ val, strBufPending, data.getJoinType());
						break;
	...
	protected int doInsertionApprTransLineHeaders(TransLineHeaderVb vObject){
		String query =  " Insert Into RA_MST_TRANS_LINE_HEADER(COUNTRY,LE_BOOK,TRANS_LINE_ID,TRANS_LINE_DESCRIPTION,TRANS_LINE_TYPE" + 
				",TRANS_LINE_PROD_SUB_TYPE,TRANS_LINE_PROD_GRP" + 
		...
		Object[] args = {vObject.getCountry(),vObject.getLeBook(),vObject.getTransLineId(),vObject.getTransLineDescription(),
				vObject.getTransLineType(),vObject.getTransLineSubType(),
				vObject.getTransLineGrp(),
	...
	public ExceptionCode doUpdateRecordForNonTrans(TransLineHeaderVb vObject) throws RuntimeCustomException  {
		...
		if (collTemp.size() > 0){
			...
			if(vObject.getTransLineGllst() != null && vObject.getTransLineGllst().size() > 0) {
				transLinesGlDao.deleteAndInsertPendGl(vObject);
			}
		 ...
		 // Record is there in approved, but not in pending.  So add it to pending
			...
			if(ValidationUtil.isValid(vObject.getBusinessVertical())) {
				transLinesSbuDao.deleteAndInsertPendSbu(vObject);//del and insert pend
			}
			if(vObject.getTransLineGllst() != null && vObject.getTransLineGllst().size() > 0) {
				transLinesGlDao.deleteAndInsertPendGl(vObject);
			}
	...
	/*public ExceptionCode doDeleteApprRecordForNonTrans(TransLineHeaderVb vObject) throws RuntimeCustomException {
	List<TransLineHeaderVb> collTemp = null;
	ExceptionCode exceptionCode = null;
	TransLineHeaderVb vObjectlocal = null;
	...
	int transLineSbuRecCnt = getTransLineSbuApprRecCnt(vObject); // Get count of existing Sbu Record
	int transLineGlCnt = getApprTransLineGlRecCnt(vObject); // Get count of existing GL Record
...
//		setStatus(vObject, Constants.PASSIVATE);
		...
		if (transLineSbuRecCnt > 0){
			//doUpdateApprTransSbu(vObjectlocal);
		}
		if (transLineGlCnt > 0){
			//doUpdateApprTransGl(vObjectlocal);
		}
	}else{
		retVal = deleteTransLineHeaderAppr(vObject);
		if (transLineSbuRecCnt > 0){
			retVal = deleteTransLineSbuAppr(vObject);
		}
		if (transLineGlCnt > 0){
			retVal = deleteTransLineGlAppr(vObject);
		}
	}
	exceptionCode = getResultObject(Constants.SUCCESSFUL_OPERATION);
	return exceptionCode;
}*/
/*public ExceptionCode doDeleteRecordForNonTrans(TransLineHeaderVb vObject) throws RuntimeCustomException {
	List<TransLineHeaderVb> collTemp = null;
	ExceptionCode exceptionCode = null;
	TransLineHeaderVb vObjectlocal = null;
	strCurrentOperation = Constants.DELETE;
...
//		setStatus(vObject, Constants.PASSIVATE);
	...
	return exceptionCode;
		...
		/*public List<TransLineHeaderVb>  getQueryResultsNavigate(TransLineHeaderVb dObj){
			List<TransLineHeaderVb> collTemp = null;
			String strApp = "";
			String strPend = "";
			...
			strApp = new String("SELECT TAppr.COUNTRY, TAppr.LE_BOOK, TAppr.TRANS_LINE_ID,TAppr.TRANS_LINE_DESCRIPTION, " + 
					...
					"TAppr.DATE_LAST_MODIFIED ,                                                                                                 " + 
					"TAppr.DATE_CREATION FROM RA_MST_TRANS_LINE_HEADER TAppr ,Alpha_sub_tab t1,ALPHA_SUB_TAB T2,NUM_SUB_TAB T3,NUM_SUB_TAB T4,  " + 
			...
			whereNotExists = new String("Not Exists (Select 'X' From RA_MST_TRANS_LINE_HEADER_PEND TPEND WHERE TAppr.COUNTRY = TPend.COUNTRY " + 
					" AND TAppr.LE_BOOK = TPend.LE_BOOK AND TAppr.TRANS_LINE_ID = TPend.TRANS_LINE_ID)");
			
			strPend = new String("SELECT TPend.COUNTRY, TPend.LE_BOOK, TPend.TRANS_LINE_ID,TPend.TRANS_LINE_DESCRIPTION,           " + 
					...
					"TPend.DATE_LAST_MODIFIED,                                                                                                 " + 
					"TPend.DATE_CREATION FROM RA_MST_TRANS_LINE_HEADER_PEND TPend ,Alpha_sub_tab t1,ALPHA_SUB_TAB T2,NUM_SUB_TAB T3,NUM_SUB_TAB T4,  " + 
			...
			//whereCondition = "WHERE COUNTRY = ? AND LE_BOOK = ? AND TRANS_LINE_ID = ?"; 
			orderBy = "ORDER BY COUNTRY , LE_BOOK, TRANS_LINE_ID";
			return getquer;
		}*/
}